...
public class KeycloakServiceImpl implements KeycloakService {
    ...
    public void syncUserTypeAttributes(UserType userType) {
        ...
        List<String> allDbTypes = userTypeRepository.findAll().stream()
                ...
                .distinct()
                .toList();
        String commaSeparatedTypes = String.join(",", allDbTypes);

        try {
            RealmRepresentation patch = new RealmRepresentation();
            Map<String, String> attr = new HashMap<>();
            attr.put("user_type_list", commaSeparatedTypes);
            patch.setAttributes(attr);


            keycloak.realm(realm).update(patch);
            log.info("Successfully patched Realm Attributes with: {}", commaSeparatedTypes);
        } catch (Exception e) {
            log.error("Safe Sync failed: {}", e.getMessage());
        }
        ...
        for (var field : userType.getFields()) {
            ...
            validatorConfig.put("required_role", typeName);
            validations.put("user-type-field-dependency", validatorConfig);
